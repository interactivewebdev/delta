...
<head>
    ...
    <!--//theme-style-->
    <script src="{{ url('assets/front/plugins/jquery/jquery.min.js') }}"></script>
...
<script>
    setTimeout(() => {
        var subscribed = "{{ isset($_COOKIE['subscribed']) ? $_COOKIE['subscribed'] : '' }}";
    ...
    function subscribe() {
        ...
        if (newObj.email != '') {
            $.ajax({
                    method: "POST",
                    url: "{{ url('/api/subscribe/newsletter') }}",
                    data: newObj
                })
                .done(function(msg) {
    ...
    function noSubscribe() {
        $.ajax({
                url: "{{ url('/nosubscribe/newsletter') }}",
            })
            .done(function(msg) {
                console.log(msg);
            });
