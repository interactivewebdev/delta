...
trait BlogTrait
{
    ...
    public function getAllBlogs($request = null)
    {
        $blogs = DB::table('news_blog as n')
        ...
        if (!is_null($request)) {
            if (trim($request['keyword']) != "") {
                $blogs = $blogs->where('n.title', 'like', '%' . trim($request['keyword']) . '%');
            }

            if ($request['sort'] == "latest") {
                $blogs = $blogs->orderBy('n.created_at', 'DESC');
            } else if ($request['sort'] == "oldest") {
                $blogs = $blogs->orderBy('n.created_at', 'ASC');
            } else {
                $blogs = $blogs->orderBy('n.order_by', 'DESC');
            };
        } else {
            $blogs = $blogs->orderBy('n.order_by', 'DESC');
        }
